                ...
                <div class="form-wrap">
                    <form id="formUpdate">
                        <div class="form-group mb-0 mb-15 with-validation tanggal" id="tanggal-group">
                            <input type="hidden" name="id" id="id" value="{{$kerusakan->id}}">
                            <label class="control-label mb-10 " for="tanggal">Tanggal</label>
                            <input type="date" id="tanggal" name="tanggal" class="form-control"
                                placeholder="tanggal" value="{{$kerusakan->tanggal}}">
                                    ...
                                    @foreach ($sarana as $sarana)
                                        <tr>
                                            <td class="text-center">
                                                <input type="checkbox" class="checkbox-sarana" {{ checkedData($kerusakan->sarana, $sarana->id)['checked'] }}
                                            ...
                                            <td>
                                                <div class="form-group mb-0 mb-15 with-validation jumlah-kerusakan" id="jumlah-kerusakan-group-{{ $sarana->id }}">
                                                    <input class="form-control jumlah-kerusakan" {{ checkedData($kerusakan->sarana, $sarana->id)['checked'] == 'checked' ? '' : 'disabled' }}
                                                        id="jumlah-kerusakan{{ $sarana->id }}" data-id="{{ $sarana->id }}"
                                                        data-jumlah="{{ $sarana->jumlah }}" name="jumlah-kerusakan[]"
                                                        data-kepemilikan="{{$sarana->kepemilikan}}" value="{{ checkedData($kerusakan->sarana, $sarana->id, 'jumlah')['data'] }}">
                                                    <div class="help-block with-errors error-message">
                                                        <ul class="list-unstyled">
                                            ...
                                            <td>
                                                <div class="form-group mb-0 mb-15 with-validation keterangan" id="keterangan-group-{{ $sarana->id }}">
                                                    <textarea class="form-control keterangan" {{ checkedData($kerusakan->sarana, $sarana->id)['checked'] == 'checked' ? '' : 'disabled' }}
                                                        id="keterangan{{ $sarana->id }}" data-id="{{ $sarana->id }}"
                                                        data-jumlah="{{ $sarana->jumlah }}" name="keterangan[]"
                                                        data-kepemilikan="{{$sarana->kepemilikan}}">{{ checkedData($kerusakan->sarana, $sarana->id, 'keterangan')['data'] }}</textarea>
                                                    <div class="help-block with-errors error-message">
                                                        <ul class="list-unstyled">
