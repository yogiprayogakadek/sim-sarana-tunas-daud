...
$(document).ready(function () {
    ...
    // on save button
    $("body").on("click", ".btn-validasi", function (e) {
        $.ajaxSetup({
            headers: {
                "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
            },
        });
        let form = $("#formValidasi")[0];
        let data = new FormData(form);
        $.ajax({
            type: "POST",
            url: "/pengembalian/validasi",
            data: data,
            processData: false,
            contentType: false,
            cache: false,
            beforeSend: function() {
                $(".btn-validasi").html("Mohon tunggu...").prop('disabled', true);
            },
            done: function () {
                $(".btn-validasi").html("Simpan").prop('disabled', false);
            },
            success: function (response) {
